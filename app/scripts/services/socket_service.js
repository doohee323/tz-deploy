...
angular.module('SdtDeploy').factory(
		...
		[
				...
				'SessionService',
				'config',
				function($rootScope, SessionService, config) {
					var socket = {};
					...
					return {
						ready : function(roomid, callback) {
							try {
								...
								var socketUrl = document.location.protocol + '//' + document.location.hostname + ':3002'
										+ '/tz_socket?roomid=' + roomid + "&userid=" + user.userid;
								if (socket[roomid]) {
						...
						off : function(eventName, callback) {
							var roomid = gf_getRoomId(eventName);
							if (roomid != 's_talk' && socket[roomid] && socket[roomid].disconnected == false
