...
angular.module('SdtDeploy').factory('CommcdCtrl', function($http) {
	...
	function query() {
		...
		$http({
			...
			if (res) {
				if (typeof res.data === 'string' && res.data.indexOf('A PHP Error was encountered') > -1) {
					res.data = res.data.substring(res.data.indexOf('{'), res.data.lastIndexOf('}') + 1);
				}
